...
it("guided full descent", () => {
  ...
  for (let i=0;i<45000 && s.terminalState===null;i++){
    ...
    t+=STEP;
    if (i%1500===0) console.log((t/1e6).toFixed(0),"s alt",o.altitudeM.toFixed(0),"vh",o.tangentialSpeedMps.toFixed(0),"vr",o.radialSpeedMps.toFixed(1),"rng",(range/1000).toFixed(1),"km thr",throttle.toFixed(2),"pitch",(s.attitudeRad*57.3).toFixed(0),"cmdPitch",(cue.recommendedAttitudeRad*57.3).toFixed(0),"mass",totalMassKg(s).toFixed(0));
