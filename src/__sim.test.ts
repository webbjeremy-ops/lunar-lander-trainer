...
import { MISSIONS, angleForRange, downrangeToLandingZoneM, LANDING_ZONE_ANGLE_RAD, nominalAltitudeForRangeM, HIGH_GATE_RANGE_M, throttleCeilingForSinceIgnition } from "@/game/play";
...
it("guided full descent", () => {
  ...
  const o=computeOrbitalValues(s);
  console.log("END", s.terminalState, "t",(t/1e6).toFixed(0),"alt",o.altitudeM.toFixed(1),"range km",(downrangeToLandingZoneM(o.centralAngleRad,LANDING_ZONE_ANGLE_RAD)/1000).toFixed(2),"vr",o.radialSpeedMps.toFixed(2),"vh",o.tangentialSpeedMps.toFixed(2),"prop",s.descentPropellantKg.toFixed(0));
});
